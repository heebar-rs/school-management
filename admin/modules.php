...
/* ---------- Charger filière ---------- */
$stmtF = $conn->prepare("SELECT id, nom_filiere, description FROM filieres WHERE id=?");
...
if(isset($_POST['add_module'])){
  ...
  $coef = trim($_POST['coefficient'] ?? '');

  $coefVal = ($coef === '') ? null : (float)$coef;

  if($nom !== ''){
    $stmt = $conn->prepare("INSERT INTO modules(filiere_id, nom_module, description, coefficient) VALUES(?,?,?,?)");
    $stmt->bind_param("issd", $filiere_id, $nom, $desc, $coefVal);
    $stmt->execute();
  }
  header("Location: modules.php?filiere_id=".$filiere_id);
  exit;
}
...
if(isset($_POST['update_module'])){
  ...
  if($id > 0 && $nom !== ''){
    $stmt = $conn->prepare("UPDATE modules SET nom_module=?, description=?, coefficient=? WHERE id=? AND filiere_id=?");
    $stmt->bind_param("ssdii", $nom, $desc, $coefVal, $id, $filiere_id);
    $stmt->execute();
  }
  header("Location: modules.php?filiere_id=".$filiere_id);
  exit;
...
if(isset($_POST['delete_module'])){
  $id = (int)($_POST['id'] ?? 0);
  if($id > 0){
    $stmt = $conn->prepare("DELETE FROM modules WHERE id=? AND filiere_id=?");
    $stmt->bind_param("ii", $id, $filiere_id);
    $stmt->execute();
  }
  header("Location: modules.php?filiere_id=".$filiere_id);
  exit;
...
/* ---------- READ MODULES ---------- */
$modules = $conn->prepare("SELECT * FROM modules WHERE filiere_id=? ORDER BY id DESC");
$modules->bind_param("i", $filiere_id);
$modules->execute();
$modulesRes = $modules->get_result();
$count = $modulesRes ? $modulesRes->num_rows : 0;
...
<title>Modules • <?= htmlspecialchars($filiere['nom_filiere']) ?></title>
...
.titleblock h2{margin:0;font-size:20px;color:var(--secondary)}
...
<header>
  <h1 style="margin-left:70px">
    Modules • <?= htmlspecialchars($filiere['nom_filiere']) ?>
  </h1>
  <div class="right">
    <span class="badge"><i class="fa-solid fa-database"></i> <?= (int)$count ?> modules</span>
    <a class="btn-light" href="filieres.php"><i class="fa-solid fa-arrow-left"></i> Retour</a>
  </div>
</header>
...
<div class="container">
  ...
  <div class="topbar">
    <div class="titleblock">
      ...
      <p>Filière: <strong><?= htmlspecialchars($filiere['nom_filiere']) ?></strong></p>
    </div>

    <button class="btn btn-primary" type="button" onclick="openAdd()">
      <i class="fa-solid fa-plus-circle"></i> Nouveau module
    </button>
  </div>

  <div class="cards">
    <?php if($count > 0): ?>
      <?php while($m = $modulesRes->fetch_assoc()): ?>
        <div class="card">
          <div class="card-head">
            <h3 class="card-title">
          ...
          <div class="card-body">
            ...
            <div class="meta">
              <span><i class="fa-solid fa-scale-balanced"></i>
                Coef: <strong><?= ($m['coefficient'] === null ? "—" : htmlspecialchars((string)$m['coefficient'])) ?></strong>
              </span>
            </div>
          </div>
          ...
          <div class="card-actions">
            <button
              ...
              type="button"
              data-id="<?= (int)$m['id'] ?>"
              data-nom="<?= htmlspecialchars($m['nom_module'], ENT_QUOTES, 'UTF-8') ?>"
              data-desc="<?= htmlspecialchars($m['description'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
              data-coef="<?= htmlspecialchars((string)($m['coefficient'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
            >
            ...
            <form method="post" onsubmit="return confirmDelete();" style="margin:0">
              ...
              <button class="btn btn-del" type="submit" name="delete_module">
...
<div class="modal" id="addModal" onclick="closeOnOutside(event,'addModal')">
  <div class="modal-card" onclick="event.stopPropagation()">
    <div class="modal-head">
      <h3><i class="fa-solid fa-plus-circle"></i> Ajouter un module</h3>
    ...
    <div class="modal-body">
      <form method="post" id="addForm">
        <div class="form-group">
          ...
          <input type="text" name="nom_module" id="add_nom" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="number" step="0.01" name="coefficient" id="add_coef" placeholder="Ex: 2">
        </div>
        <div class="modal-foot">
          <button class="btn btn-light" type="button" onclick="closeAdd()">Annuler</button>
          <button class="btn btn-primary" type="submit" name="add_module">
...
<div class="modal" id="editModal" onclick="closeOnOutside(event,'editModal')">
  <div class="modal-card" onclick="event.stopPropagation()">
    <div class="modal-head">
      <h3><i class="fa-solid fa-pen"></i> Modifier le module</h3>
    ...
    <div class="modal-body">
      <form method="post" id="editForm">
        ...
        <div class="form-group">
          <label>Nom du module</label>
          <input type="text" name="nom_module" id="edit_nom" required>
        </div>
        <div class="form-group">
          <label>Description</label>
        ...
        <div class="form-group">
          <label>Coefficient (optionnel)</label>
          <input type="number" step="0.01" name="coefficient" id="edit_coef">
        </div>
        <div class="modal-foot">
          <button class="btn btn-light" type="button" onclick="closeEdit()">Annuler</button>
          <button class="btn btn-primary" type="submit" name="update_module">
...
function openAdd(){
  ...
  setTimeout(()=>document.getElementById('add_nom').focus(), 30);//focus curseur dans input
}
function closeAdd(){
...
function openEdit(id, nom, desc, coef){
...
function confirmDelete(){
  return confirm("Supprimer ce module ? (Action irréversible)");
}

function closeOnOutside(e, id){//fermer une modale si on clique à l’extérieur.
  if(e.target.id === id){// e target = élément cliqué exact
    document.getElementById(id).classList.remove('active');
    if(id === 'addModal') document.getElementById('addForm').reset();
  }
}

/* Boutons Modifier (data-*) */
document.querySelectorAll('.js-edit').forEach(btn=>{//parcourir chaque bouton avec la classe js-edit
  btn.addEventListener('click', ()=>{//Ajoute un écouteur :quand on clique sur ce bouton → exécute le code.
    openEdit(btn.dataset.id, btn.dataset.nom, btn.dataset.desc, btn.dataset.coef);//lire data-*
  });
});
